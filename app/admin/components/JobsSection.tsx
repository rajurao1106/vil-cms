  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ jobTitle: "", department: "", location: "", jobType: "Full-time", isActive: true, description: "", requirements: "" });
        fetchJobs();
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-12">
      ...
      <section className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        <h2 className="text-2xl font-bold mb-6 text-teal-900">Post a New Career Opening</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-3 bg-teal-50 p-3 rounded-xl">
            <input
              ...
              onChange={handleChange}
            />
            <label htmlFor="isActive" className="text-teal-800 font-medium cursor-pointer">
      ...
      {/* --- JOBS LIST --- */}
      <section>
        <h2 className="text-3xl font-bold mb-8 text-gray-800">Current Openings</h2>
        <div className="grid grid-cols-1 gap-4">
          {loading ? (
            <p className="text-center text-gray-400">Loading careers...</p>
          ) : (
            jobs.map((job) => (
              <div key={job._id} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-xl font-bold text-gray-900">{job.jobTitle}</h3>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${job.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                  ...
                  <p className="text-sm text-gray-400 line-clamp-1 italic">{job.description}</p>
                </div>

                <div className="flex items-center gap-4">
                   <div className="text-right hidden md:block">
                     <p className="text-xs text-gray-400 uppercase tracking-wider">Type</p>
                     <p className="font-semibold text-gray-700">{job.jobType || "Full-time"}</p>
                   </div>
