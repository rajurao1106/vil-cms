...
export default function StatsSection() {
  ...
  const removeStatRow = (index: number) => {
    ...
    setStats(updatedStats.length > 0 ? updatedStats : [{ title: "", value: "", unit: "", order: 0 }]);
  };
            ...
            {stats.map((stat, index) => (
              <div key={index} className="flex items-start gap-3 group">
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 flex-grow">
                  {[
                    { label: "Title", field: "title", type: "text", placeholder: "Happy Clients" },
                    ...
                    <div key={input.field} className="flex flex-col">
                      ...
                      <input
                        ...
                        onChange={(e) => handleChange(index, input.field as keyof Stat, e.target.value)}
