...
export default function PostsSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Post created!");
        setFormData({ title: "", slug: "", type: "News", status: "Published", author: "", excerpt: "", content: "", coverImage: "", tags: "" });
        fetchPosts();
      }
    } catch (err) {
      ...
      <section>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {posts.map((post) => (
              <div key={post._id} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 flex flex-col">
                {post.coverImage && (
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {post.tags?.map((tag, i) => (
                      <span key={i} className="text-[10px] bg-gray-100 px-2 py-0.5 rounded text-gray-500">
                        #{tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-50 flex justify-between items-center text-sm">
                    <span className="text-gray-500 italic">By {post.author || "Admin"}</span>
